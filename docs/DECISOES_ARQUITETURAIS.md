...
## ADR-009: Refatoração do FeedService para Respeitar SRP

**Status**: Aceito  
**Data**: 2025-01  
**Contexto**: O `FeedService` original tinha 12 dependências e violava o Single Responsibility Principle, sendo responsável por criação, listagem, interações e filtragem de posts.

**Decisão**: Refatorar `FeedService` em três services especializados:
- `PostCreationService`: Responsável apenas pela criação de posts (validações, geo anchors, assets, eventos)
- `PostInteractionService`: Responsável apenas por interações (likes, comentários, shares)
- `PostFilterService`: Responsável apenas por filtragem e paginação (visibilidade, bloqueios, filtros por entidade/asset)
- `FeedService`: Mantido como orquestrador/facade que delega aos services especializados

**Consequências**:
- ✅ Redução de dependências de 12 para 4 no `FeedService`
- ✅ Melhor separação de responsabilidades (SRP)
- ✅ Código mais testável e manutenível
- ✅ Facilita evolução independente de cada funcionalidade
- ✅ Redução de complexidade ciclomática
- ⚠️ Necessidade de atualizar testes (resolvido com `FeedServiceTestHelper`)
- ⚠️ Mais classes para gerenciar (trade-off aceito)

**Alternativas Consideradas**:
1. Manter `FeedService` monolítico (rejeitado - viola SRP, difícil de manter)
2. Refatorar apenas criação (rejeitado - não resolve problema completo)
3. Usar padrão Strategy (rejeitado - over-engineering para este caso)

**Implementação**:
- Services criados em `backend/Araponga.Application/Services/`
- `FeedService` refatorado para usar injeção de dependência dos novos services
- Configuração de DI atualizada em `ServiceCollectionExtensions`
- Helper de testes criado para facilitar instanciação nos testes
