...
- **Hardening (revisão de bots #431)**: confirmação valida que o pagamento pertence ao checkout (`PaymentStatusResult.CheckoutId`); estorno marca `Refunded` no mesmo commit da reversão do ledger (atomicidade/idempotência) e persiste status `Completed` das transações de estorno antes do `AddAsync`; `MockPaymentGateway` singleton e thread-safe (estado sobrevive entre requisições); `IPaymentGateway` mock só fora de produção (`UnavailablePaymentGateway` em produção); `PostgresFeeSplitRuleRepository` compara `RevenueType` case-insensitive
