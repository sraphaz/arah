...
Comandos:
  ...
  github-project  Project v2 — bootstrap | sync-backlog | sync-project | sync-status | dedupe-backlog | reconcile | status (-Skill)
...
switch ($Command) {
    ...
    'github-project' {
        $sub = 'help'
        if ($Skill -in @('bootstrap', 'ensure', 'sync-backlog', 'sync-queue', 'sync-project', 'sync-status', 'dedupe-backlog', 'reconcile', 'status', 'help')) { $sub = $Skill }
        $params = @{ Command = $sub }
